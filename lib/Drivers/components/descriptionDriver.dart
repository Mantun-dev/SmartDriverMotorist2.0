...
// ignore: missing_return
  Future<Salida> fetchAgentsLeftPastToProgres(
      String hourOut, String nameController) async {
    http.Response responses = await http
        .get(Uri.parse('$ip/apis/refreshingAgentData/${prefs.nombreUsuario}'));
    ...
    if (responsed.statusCode == 200) {
      ...
      if (data1.type == "error") {
        SweetAlert.show(
          ...
          title: '¡No encontrado!',
          subtitle:
              'No se encontró el agente con número de empleado \n $agentEmployeeId',
          style: SweetAlertStyle.error,
        );
  ...
  Widget _mostrarQuintaVentana(BuildContext context) {
    return Column(
      children: [
        ...
        SizedBox(height: 6.0),
        FutureBuilder(
            future: this.handler.retrieveAgent() == null
                ? noemp.length
                : this.handler.retrieveAgent(),
            builder:
                (BuildContext context, AsyncSnapshot<List<User>> snapshot) {
              if (snapshot.hasData) {
                return Container(
