...
Future scanBarcodeNormalSolid() async {
  ...
  final tables2 = await db.rawQuery("SELECT noempid FROM agentInsert WHERE noempid = '${prefs.nameSalida}'");
  http.Response responsed = await http.post(Uri.encodeFull('$ip/apis/getAgentForEntryTrip'), body: data);
  ...
  if (responsed.statusCode == 200 ) {
    if (barcodeScan == '${-1}') {
      print('');
        ...
        SweetAlert.show(context,
          title: '¡No encontrado!',
          subtitle: 'No se encontró el agente con número de empleado \n $agentEmployeeId',
          style: SweetAlertStyle.error,
        );
                ...
                child: Column(
                  children: <Widget>[
                    ...
                    ListTile(
                          contentPadding: EdgeInsets.fromLTRB(5, 5, 5, 0),
                          title: Text('Dirección: '),
                          subtitle: Text('${data1.agent.neighborhoodName} ${data1.agent.agentReferencePoint}\n${data1.agent.departmentName} '),
                          leading: Icon(Icons.directions , color: kColorAppBar),
                    ),
                    SizedBox(height: 40),
                    Row(
                            ...
                            setState(() {                                                          
                              if (tables.length <= 13) {          
                                if (prefs.nameSalida != tables2) {
                                    noemp.insert(0, '${data1.agent.agentEmployeeId}');
                                    names.insert(0,'${data1.agent.agentFullname}');
                                    hourout.insert(0, '${data1.agent.hourAgent}');
                                    direction.insert(0, '${data1.agent.agentReferencePoint} ${data1.agent.neighborhoodName}\n${data1.agent.departmentName}');
                                    tempArr.add(data1.agent.agentId);
                                    prefs.destinationIdAgent = data1.agent.companyId.toString();
                                    prefs.nameSalida = data1.agent.agentEmployeeId.toString();
                                    User firstUser = User(noempid: '${data1.agent.agentEmployeeId}', nameuser: '${data1.agent.agentFullname}', hourout: '${data1.agent.hourAgent}',
                                    direction:'${data1.agent.departmentName} ${data1.agent.neighborhoodName}\n${data1.agent.agentReferencePoint}', 
                                    idsend:data1.agent.agentId );
                                    print(firstUser);
                                    List<User> listOfUsers = [firstUser];
...
Future<List<Destinationations>>fetchDestinations()async{
  http.Response response = await http.get(Uri.encodeFull('$ip/apis/refreshingAgentData/${prefs.nombreUsuario}'));
  final si = DriverData.fromJson(json.decode(response.body));
  http.Response responsed = await http.get(Uri.encodeFull('$ip/apis/getDestinationsForEntryTrip/${si.driverId}'));
    var jsonData = json.decode(responsed.body);      
    List< Destinationations> trips = [];
    for (var u in jsonData['destinations']) {
      Destinationations trip = Destinationations(u["destinationId"], u["destinationName"]);
      trips.add(trip);
    }
    if (mounted) {
      setState(() {
    ...
    final tables = await db.rawQuery('SELECT * FROM agentInsert ;');   
      if (tables.length == 0) {
        ...
        if (si.driverCoord == true) {
          ...
          http.Response response1 = await http.post(Uri.encodeFull('$ip/apis/registerTripEntryByDriver'), body: datas);
          ...
          for (var i = 0; i < tables.length; i++) {
            ...
            if (response1.statusCode == 200) {             
            final sendDatas = await http.post(Uri.encodeFull('$ip/apis/registerAgentTripEntryByDriver'), body: datas2);
              ...
              SweetAlert.show(context,
                title: '¡Éxito!',
                subtitle: 'El viaje se ha registrado correctamente',
                style: SweetAlertStyle.success,
              );
          ...
          for (var i = 0; i < tables.length; i++) {
            Map datas2 = {
              ...
              await http.post(Uri.encodeFull('$ip/apis/registerAgentTripEntryByDriver'), body: datas2);                        
              Navigator.pop(context);
              Navigator.push(context,MaterialPageRoute(builder: (context) => MyConfirmAgent(),));
              prefs.removeIdCompanyAndVehicle();
              SweetAlert.show(context,
      ...
      child: Row(
        children: [
          ...
          Expanded(child:  new DropdownButton(
            ...
            items: data2.map((e) {
              return new DropdownMenuItem(
                child: Text(e['destinationName']==null||e['destinationName']==""?prefs.destinationPrueba:e['destinationName']),
  ...
  Widget _mostrarQuintaVentana(BuildContext context) {
     return Column(
          children: [
            ...
            SizedBox(height: 6.0),
            FutureBuilder(
                future: this.handler.retrieveAgent() == null?noemp.length:this.handler.retrieveAgent(),
                builder: (BuildContext context, AsyncSnapshot<List<User>> snapshot){
                  if (snapshot.hasData) { 
                    return Container(
                      child: Column(
